...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	parsePrefixExp := p.prefixParseFns[p.curToken.Type]
	if parsePrefixExp == nil {
		errMsg := fmt.Sprintf("no prefix parser found for %s", p.curToken.Type)
		p.errors = append(p.errors, errMsg)
		return nil
	}
	leftExp := parsePrefixExp()

	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		parseInfixExp := p.infixParseFns[p.peekToken.Type]
		if parseInfixExp == nil {
			return leftExp
		}

		p.nextToken()
		leftExp = parseInfixExp(leftExp)
	}
...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	...
	if err != nil {
		msg := fmt.Sprintf("could not parse %s as an integer: %s", p.curToken.Literal, err)
		p.errors = append(p.errors, msg)
	}
